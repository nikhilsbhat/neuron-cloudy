...
type ServerCreateInput struct {
	// InstanceName refers to the name that has to be assigend to the instance which will be created.
	InstanceName string `json:"instancename"`
	// Count defines the number of instance that has to be created.
	Count int64 `json:"count"`
	// Id of the base image that has to be used for creating the vm's.
	ImageId string `json:"imageid"`
	// SubnetId is the ID of the subnetwork in which the instance/vm has to be created.
	SubnetId string `json:"subnetid"`
	// KeyName of the ssh keypair that has to used for creation of vm's, and this key will be used to log into the servers.
	...
	// UserData refers to the raw codes that has to be executed immediately after server boots up goes here.
	UserData string `json:"userdata"`
	// AssignPubIp defines whether a public IP has to be assigned to VM or not
